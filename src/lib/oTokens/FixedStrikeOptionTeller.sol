...
import {Auth, Authority} from "solmate-6.8.0/auth/Auth.sol";
import {ClonesWithImmutableArgs} from "./clones/ClonesWithImmutableArgs.sol";

import {IFixedStrikeOptionTeller, IOptionTeller} from "./IFixedStrikeOptionTeller.sol";
...
contract FixedStrikeOptionTeller is IFixedStrikeOptionTeller, Auth, ReentrancyGuard {
    ...
    function deploy(
        ...
        bytes32 optionHash = _getOptionTokenHash(
            payoutToken_, quoteToken_, eligible_, expiry_, receiver_, call_, strikePrice_
        );
    ...
    function _deploy(
        ...
        (bytes32 name, bytes32 symbol) =
            _getNameAndSymbol(payoutToken_, quoteToken_, expiry_, call_, strikePrice_);

        // Deploy option token
        return FixedStrikeOptionToken(
    ...
    function create(
        ...
        FixedStrikeOptionToken optionToken =
            getOptionToken(payoutToken, quoteToken, eligible, expiry, receiver, call, strikePrice);
    ...
    function getOptionToken(
        ...
        bytes32 optionHash = _getOptionTokenHash(
            payoutToken_, quoteToken_, eligible, expiry, receiver_, call_, strikePrice_
        );
    ...
    function _getNameAndSymbol(
        ...
        string memory dayStr;
        {
            ...
            yearStr = _uint2str(uint256(_year) % 10_000);
        ...
        bytes32 name = bytes32(
            abi.encodePacked(
                tokenSymbols, " ", callPut, " ", strike, " ", yearStr, monthStr, dayStr
            )
        );
        bytes32 symbol =
            bytes32(abi.encodePacked("o", payoutSymbol, "-", yearStr, monthStr, dayStr));
